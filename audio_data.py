...
class WavenetDataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        if not os.path.isfile(dataset_file): # 如果没有npy文件，就从目录下生成
    ...
    def create_dataset(self, location, out_file):
        # 把location里的音频文件全部转化为0~255的数字表示
    ...
    def calculate_length(self):
        # 计算每个音频文件的起始位置
        start_samples = [0]
        for i in range(len(self.data.keys())): # 遍历所有的音频文件数据
    ...
    def __getitem__(self, idx):
        ...
        # a.shape is 256xlen 整个输入长度 , target.shape is 1xoutput_len 即后output项，输出长度
