...
class WaveNetModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.end_conv_2 = nn.Conv1d(in_channels=end_channels,
                                    out_channels=classes,  # 在这里修改成41类
                                    kernel_size=1,
                                    bias=True)
    ...
    def wavenet(self, input, dilation_func):
        ...
        for i in range(self.blocks * self.layers):
            ...
            #print('every block',x.shape)

        # 可以去掉下面的卷积 加上平均池化后，加上几个卷积
        #print('skip ',skip.shape)  [N, 512 32]
    ...
    def forward(self, input): # [256,11264]
        # 不是一个点一个点地判断生成，是target个点一组地生成, 写在里面的for循环里了
        skip = self.wavenet(input,
                         dilation_func=self.wavenet_dilate)

        # this is the way to raw loss
        x = F.relu(skip)
        x = F.relu(self.end_conv_1(x))  # [N, 512, item_length]

        x = self.end_conv_2(x)  # [N, 512, item_length]
        [n, c, l] = x.size()
        l = self.output_length
        x = x[:, :, -l:]    # 取生成的后l个样本点
        x = x.transpose(1, 2).contiguous()
        x = x.view(n * l, c)    #   n个生成的target拼接在一起，[长度，256]
        # this is the way to 41 classes
        z = F.relu(skip)
        z = x[:, :, -l:]    # 取生成的后l个样本点
        z = F.avg_pool1d(z,kernel_size=160, stride=40) # [N, 256, 125]
        z = z.view(n,256*125)
        z = self.classifier(z) # [N, 41]
        # [N*target_length,256] [N,41]
        return x, z
